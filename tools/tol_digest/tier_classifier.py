...
def classify_claim(text:str):
    l=text.lower(); unsafe=[]
    if any(k in l for k in ['phase conjugate trauma deletion','trauma deletion','guaranteed healing','proof of immortality','instant arhat','clinical bypass','theory validated','proves language']):
        return 'unsafe_or_requires_rewrite','unsafe_until_rewritten',True,['unsafe']
    if any(k in l for k in ['soul is real','proves','karmic deletion','immortal','vacuum','topological braid','topological afterlife','kl1','quantum-vacuum']):
