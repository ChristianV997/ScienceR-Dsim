...
from sciencer_d.btc_icft.level_t import ds003816_real_topology as topo


def _m_rows():
    return [
        {"row_id":"r1","subject_id":"sub-01lt","session_id":"ses-01","run_id":"","window_id":"w1","task_label":"LKMSelf","source_file":"a","window_start_s":"0","window_end_s":"10","artifact_score":"0.1"},
        {"row_id":"r2","subject_id":"sub-02st","session_id":"ses-01","run_id":"","window_id":"w2","task_label":"PreResting","source_file":"b","window_start_s":"10","window_end_s":"20","artifact_score":"0.6"},
    ]
...
def test_missing_features_raises(tmp_path: Path):
    try:
        ...
        assert "Run run_ds003816_m_real first or use --mock-fixture" in str(e)
...
def test_reports_and_outputs(tmp_path: Path):
    ...
    res = topo.LevelTRealTopologyResult("ds003816", len(rows), 2, len(rows), q, n, a, topo.build_level_t_omega_event(rows),
        "Local DS003816-style EEG windows were mapped into operational Level T topology telemetry candidates for future M+T residual testing.",
        ["No topology proof."], [])
...
def test_cli(tmp_path: Path):
    c = [sys.executable, "-m", "sciencer_d.btc_icft.pipelines.run_ds003816_t_real", "--out", str(tmp_path / "o"), "--mock-fixture"]
    r = subprocess.run(c, capture_output=True, text=True)
    assert r.returncode == 0

    # neither --real nor --mock-fixture: must fail loudly, not silently pick one
    c2 = [sys.executable, "-m", "sciencer_d.btc_icft.pipelines.run_ds003816_t_real", "--m-windows", str(tmp_path / "nope"), "--out", str(tmp_path / "o2")]
    ...
    c3 = [sys.executable, "-m", "sciencer_d.btc_icft.pipelines.run_ds003816_t_real", "--m-windows", str(tmp_path / "nope"), "--out", str(tmp_path / "o3"), "--real"]
    r3 = subprocess.run(c3, capture_output=True, text=True)
    assert r3.returncode != 0
    assert "Run run_ds003816_m_real first or use --mock-fixture" in (r3.stdout + r3.stderr)

    # --real and --mock-fixture together: rejected as contradictory
    c4 = [sys.executable, "-m", "sciencer_d.btc_icft.pipelines.run_ds003816_t_real", "--out", str(tmp_path / "o4"), "--real", "--mock-fixture"]
    ...
    with (m_dir / "features_m.csv").open("w", newline="", encoding="utf-8") as f:
        ...
        w.writeheader()
        w.writerows(_m_rows())
    c5 = [sys.executable, "-m", "sciencer_d.btc_icft.pipelines.run_ds003816_t_real", "--m-windows", str(m_dir), "--out", str(tmp_path / "o5"), "--real"]
...
def test_real_topology_derived_from_signal_not_metadata(tmp_path: Path, monkeypatch):
    ...
    row_a = {"row_id": "ra", "subject_id": "sub-01lt", "session_id": None, "run_id": None,
              "window_id": "w0", "task_label": "LKMSelf", "source_file": str(f1),
              "window_start_s": "0", "window_end_s": "1"}
    row_b = {"row_id": "rb", "subject_id": "sub-01lt", "session_id": None, "run_id": None,
              "window_id": "w1", "task_label": "PreResting", "source_file": str(f2),
              "window_start_s": "0", "window_end_s": "1"}
...
def test_config_contains_contract():
    t = Path("configs/btc_icft/ds003816_t_real.yaml").read_text(encoding="utf-8")
