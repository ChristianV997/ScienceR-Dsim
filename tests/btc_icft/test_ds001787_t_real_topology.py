...
def _m_rows():
    return [
        {"row_id":"r1","subject_id":"sub-001","session_id":"ses-01","run_id":"","window_id":"w1","task_label":"meditation","source_file":"a","window_start_s":"0","window_end_s":"10","artifact_score":"0.1"},
        {"row_id":"r2","subject_id":"sub-002","session_id":"ses-01","run_id":"","window_id":"w2","task_label":"meditation","source_file":"b","window_start_s":"30","window_end_s":"165","artifact_score":"0.6"},
    ]
...
def test_missing_features_raises(tmp_path: Path):
    try:
        ...
        assert "Run run_ds001787_m_real first or use --mock-fixture" in str(e)
...
def test_reports_and_outputs(tmp_path: Path):
    ...
    res = topo.LevelTRealTopologyResult("ds001787", len(rows), 2, len(rows), q, n, a, topo.build_level_t_omega_event(rows),
        "Local DS001787-style EEG windows were mapped into operational Level T topology telemetry candidates for future M+T residual testing.",
        ["No topology proof."], [])
...
def test_cli(tmp_path: Path):
    c = [sys.executable, "-m", "sciencer_d.btc_icft.pipelines.run_ds001787_t_real", "--out", str(tmp_path / "o"), "--mock-fixture"]
    r = subprocess.run(c, capture_output=True, text=True)
    assert r.returncode == 0

    c2 = [sys.executable, "-m", "sciencer_d.btc_icft.pipelines.run_ds001787_t_real", "--m-windows", str(tmp_path / "nope"), "--out", str(tmp_path / "o2")]
    ...
    c3 = [sys.executable, "-m", "sciencer_d.btc_icft.pipelines.run_ds001787_t_real", "--m-windows", str(tmp_path / "nope"), "--out", str(tmp_path / "o3"), "--real"]
    r3 = subprocess.run(c3, capture_output=True, text=True)
    assert r3.returncode != 0
    assert "Run run_ds001787_m_real first or use --mock-fixture" in (r3.stdout + r3.stderr)

    c4 = [sys.executable, "-m", "sciencer_d.btc_icft.pipelines.run_ds001787_t_real", "--out", str(tmp_path / "o4"), "--real", "--mock-fixture"]
    ...
    with (m_dir / "features_m.csv").open("w", newline="", encoding="utf-8") as f:
        ...
        w.writeheader()
        w.writerows(_m_rows())
    c5 = [sys.executable, "-m", "sciencer_d.btc_icft.pipelines.run_ds001787_t_real", "--m-windows", str(m_dir), "--out", str(tmp_path / "o5"), "--real"]
    ...
    assert all(row["q_abs"] == "0.0" for row in out_rows)


def test_real_topology_derived_from_signal_not_metadata(tmp_path: Path, monkeypatch):
    """Same regression as ds005620/ds003969, plus: verifies variable-length windows
    (this dataset's windows span 1s-165s+, unlike prior datasets' fixed 4-10s) both
    still produce signal-derived, not metadata-derived, output.
    """
    import numpy as np
    ...
    signals = {
        ...
        str(f2): np.array([[float(i % 7) for i in range(400)] for _ in range(6)]),  # long window
    }
    monkeypatch.setattr(
    ...
    row_a = {"row_id": "ra", "subject_id": "sub-001", "session_id": "ses-01", "run_id": None,
              "window_id": "w0", "task_label": "meditation", "source_file": str(f1),
              "window_start_s": "0", "window_end_s": "1"}
    row_b = {"row_id": "rb", "subject_id": "sub-001", "session_id": "ses-01", "run_id": None,
              "window_id": "w1", "task_label": "meditation", "source_file": str(f2),
              "window_start_s": "0", "window_end_s": "160"}
...
def test_config_contains_contract():
    t = Path("configs/btc_icft/ds001787_t_real.yaml").read_text(encoding="utf-8")
