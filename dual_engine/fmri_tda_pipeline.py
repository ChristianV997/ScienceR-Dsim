...
def main(argv=None) -> int:
    ...
    # Real run: discover sub-*/func/*_bold.nii.gz under bids_root, infer group from prefix.
    ...
    for sub in sorted(root.glob("sub-*")):
        ...
        if not func:
            continue
        name = sub.name.replace("sub-", "")
        group = "CTL" if "CTL" in name else "ODN" if "ODN" in name else "ODP" if "ODP" in name else "UNK"
        results.append(run_subject(sub.name, group, str(func[0]),
                                   atlas_labels_img=args.atlas, t_r=args.t_r,
                                   provenance="real_fmri"))
        print(f"  {sub.name} ({group}): b1={results[-1].betti1} err={results[-1].error[:40]}")
    out = Path(args.out); out.mkdir(parents=True, exist_ok=True)
    stats = {m: group_compare(results, m) for m in
             ("betti1", "total_persistence_h1", "modularity", "global_efficiency")}
    (out / "cohort_result.json").write_text(json.dumps(
        {"pipeline": "fmri_tda_v1", "provenance": "real_fmri",
         "subjects": [r.to_dict() for r in results], "group_stats": stats}, indent=2), encoding="utf-8")
